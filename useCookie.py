...
# 配置日志
logging.basicConfig(
    level=logging.WARNING,  # 将日志级别从INFO改为WARNING
...
# 设置自定义logger，允许重要信息输出
logger = logging.getLogger('cookie_script')
...
def extract_cookies(curl_output: str) -> Dict[str, str]:
    ...
    if not cookies:
        logging.debug("未通过行查找到cookies，尝试使用正则表达式")
    ...
    # 直接提取session cookie
    if not cookies:
        logging.debug("正则表达式也未找到cookies，尝试直接提取session")
...
def extract_response_body(curl_output: str) -> str:
    ...
    if len(parts) > 1:
        ...
        logging.debug("响应体不包含JSON数据或格式不正确")
        return body
    logging.debug("响应体为空或未找到HTTP头部与响应体的分隔")
        ...
        if json_start >= 0 and json_end > json_start:
            ...
            if 'data' in response_data:
                ...
                if isinstance(data, dict) and 'id' in data:
                    ...
                    logging.debug("data中不包含id字段")
            else:
                logging.debug("响应中不包含data字段")
    except json.JSONDecodeError as e:
        logging.debug(f"JSON解析错误: {str(e)}")
        # 尝试直接用正则表达式提取ID
        try:
            ...
            if id_match:
                ...
                logging.debug("正则表达式未能匹配到ID")
        except Exception as regex_error:
            logging.debug(f"正则表达式提取错误: {str(regex_error)}")
    except Exception as e:
        logging.debug(f"提取用户ID时发生错误: {str(e)}")
...
def get_cookies_with_curl(username: str, password: str, batch_id: int = 0, max_retries: int = 3) -> Dict:
    ...
    for attempt in range(max_retries):
        try:
            logger.info(f"[批次{batch_id}] 正在请求账号: {username} (尝试 {attempt + 1}/{max_retries})")
            ...
            if process.returncode != 0:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.info(f"[批次{batch_id}] 等待 {backoff:.1f} 秒后重试")
                    time.sleep(backoff)
                    continue
            ...
            logger.info(f"[批次{batch_id}] 账号 {username} 响应状态码: {status_code}")
            ...
            if status_code == 200:
                ...
                if user_id:
                    logger.info(f"[批次{batch_id}] 账号 {username} 获取到用户ID: {user_id}")
                ...
                # 指数退避算法处理限流
                backoff = calculate_backoff_time(attempt)
                logger.warning(f"[批次{batch_id}] 账号 {username} 触发限流，等待 {backoff:.1f} 秒后重试")
                time.sleep(backoff)
...
def save_progress(results: List[Dict], filename: str = 'cookies_progress.json'):
    ...
    logger.info(f"当前进度已保存到 {filename}")
...
def main():
    ...
    try:
        ...
        logger.info(f"成功加载 {total_accounts} 个账号")
        ...
        logger.info(f"账号已分为 {len(batches)} 个批次，每批 {batch_size} 个账号")
            ...
            while batch_futures:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        logger.info(f"批次 {batch_id} 完成处理，成功: {sum(1 for r in batch_results if r.get('status') == 200)} 个，失败: {sum(1 for r in batch_results if r.get('status') != 200 or 'error' in r)}")
        ...
        logger.info(f"所有批次处理完成，共处理 {len(all_results)} 个账号")
        
        # 保存完整结果
        try:
            with open('cookies_results.json', 'w', encoding='utf-8') as f:
                json.dump(all_results, f, ensure_ascii=False, indent=2)
            logger.info("结果已保存到 cookies_results.json")
        except Exception as e:
        ...
        try:
            ...
            logger.info("cookies和用户ID已单独保存到 cookies_only.json")
        except Exception as e:
